...
function PostAmigos() {
  ...
  return (
    <>
      {posts.map(post => (
        <div className="bg-light p-2 rounded-3 border border-1 mb-3" key={post.post_id}>
          <div className="mb-3 d-flex">
            <div className="me-2">
              <a href={`/user/${post.user_id}`}>
                <img className="rounded-circle" src={post.image} height="50" alt="" />
              </a>
            </div>
          ...
          <div className="nav w-100">
            ...
            <div className="hstack gap-3">
              <button className="btn btn-outline-danger border-0 rounded-circle corazon" id={`boton_likes${post.id}`} type="submit">
          ...
          <div>
            <p id={`total_likes${post.post_id}`} className="ms-1"> 1582</p>
          </div>
        </div>
      ))}
    </>
